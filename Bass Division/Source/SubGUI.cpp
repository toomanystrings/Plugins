...
//==============================================================================
SubGUI::SubGUI(BassDivisionAudioProcessor& audioProcessor) : BDPanelBase(audioProcessor)//, audioProcessor(*audioProcessor)
{
    ...
    //attachLabel(attackLabel, "Attack", attackSlider);

	attackSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_ATTACK", attackSlider);
	...
	releaseSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_RELEASE", releaseSlider);
	...
	ratioSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_RATIO", ratioSlider);
	...
	//threshLabel.setFont(norse);
	addAndMakeVisible(threshLabel);

	threshSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_THRESHOLD", threshSlider);
	...
	//inputLabel.setFont(norse);
	addAndMakeVisible(inputLabel);

	inputSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_INPUT", inputSlider);
	...
	//outputLabel.setFont(norse);
	addAndMakeVisible(outputLabel);

	outputSliderAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(audioProcessor.treeState, "SUB_COMP_OUTPUT", outputSlider);
	...
	soloButtonAttachment = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(audioProcessor.treeState, "SUB_SOLO", soloButton);
}
...
void SubGUI::paint (juce::Graphics& g)
{
	//BDPanelBase::paint(g);

	g.setFont(DVFonts::getBoldNorse(25));
	//g.setFont(25);
	g.drawText("SUB", 0, 0, getWidth(), getHeight() - 15, juce::Justification::centredBottom);
}
...
void SubGUI::buttonClicked(juce::Button* button)
{
}

void SubGUI::timerCallback()
{
	
}

void SubGUI::attachLabel(juce::Label label, juce::String string, juce::Component toAttachTo)
{
    label.setText(string, juce::dontSendNotification);
    label.attachToComponent(&toAttachTo, false);
