/*
  ==============================================================================

    BDLeftPanel.cpp
    Created: 20 Feb 2021 8:55:39pm
    Author:  Olorin

  ==============================================================================
*/

#include "BDProcessPanel.h"



BDProcessPanel::BDProcessPanel(BassDivisionAudioProcessor& inProcessor)
: BDPanelBase(inProcessor), mEqPanel(inProcessor), mCompPanel(inProcessor)
{
    setSize(PROCESS_PANEL_WIDTH, PROCESS_PANEL_HEIGHT);

    addAndMakeVisible(mEqPanel);
    addAndMakeVisible(mCompPanel);
}

BDProcessPanel::~BDProcessPanel()
{
}

void BDProcessPanel::paint(juce::Graphics& g)
{
    //BDPanelBase::paint(g);

    /*g.setColour(juce::Colours::black);
    g.setFont(14);
    g.drawText("Check the area", 0, 0, getWidth(), getHeight(), juce::Justification::centred);*/
}

void BDProcessPanel::resized()
{
    auto area = getLocalBounds();

    auto eqArea = area.removeFromLeft(PROCESS_PANEL_WIDTH / 2);
    mEqPanel.setBounds(eqArea);
    mCompPanel.setBounds(area);
}
